...
def animation_plot_with_offset(animations, interval=8.33, H36 = False, repeat=False,
        output_video=None, _offsets = None, no_root_motion=True):
    assert len(animations) == len(_offsets) 
    footsteps = []
    ...
    def animate(i):
        ...
        for ai in range(len(animations)):
            ...
            offsetx = 0.0 #25*(ai-((len(animations)))/2.0)
            offsety = 0.0 #25*(ai-((len(animations)))/2.0)
            if _offsets is not None:
                offsetx, offsety = _offsets[ai]
            ...
            for j in range(len(parents)):
                if parents[j] != -1:
                    lines[ai][j].set_data(
                        [ animations[ai][ii,j,0]+offsetx,
                            animations[ai][ii,parents[j],0]+offsetx],
                        [-animations[ai][ii,j,2]+offsety,
                            -animations[ai][ii,parents[j],2] + offsety])
                    lines[ai][j].set_3d_properties(
                        [ animations[ai][ii,j,1] + offsety,
                            animations[ai][ii,parents[j],1] + offsety])
            changed += lines
...
def process_file(filename, window=240, window_step=120, export_trajectory=False, H36 = False):
    ...
    global global_across
    if global_across is not None:
        across = global_across
    else:
        global_across = across
...
def process_animation(anim, window=240, window_step=120, export_trajectory=False, H36 = False ):
    ...
    positions[:,:,0] = positions[:,:,0] - positions[:,0:1,0]
    positions[:,:,2] = positions[:,:,2] - positions[:,0:1,2]
    #print positions[10, 0, :], positions.shape
    ...
    target = np.array([[0,0,1]]).repeat(len(forward), axis=0)
    rotation = Quaternions.between(forward, target)[:,np.newaxis]
    #print 'rotation', type(rotation[0])
    positions = rotation * positions
    
    """ Get Root Rotation """
    velocity = rotation[1:] * velocity
    rvelocity = (rotation[1:] * -rotation[:-1]).to_pivots()
    #rvelocity = np.zeros_like(rvelocity)
    ...
    #print windows[0].shape, anim.shape
...
def aligned_plot(src, tgts, align, interval=8.33, H36 = False, repeat=True,
    ...
    def animate(i):
        ...
        for count, pair in enumerate(align[ii]):
            ...
            tnum, fnum = pair
            tnum += 1
            offset = 0 #25*(ai-((len(animations)))/2.0)
            for j in range(len(parents)):
                if parents[j] != -1:
                    #print animations[tnum].shape, fnum, tnum
