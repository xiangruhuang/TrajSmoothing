...
def load(filename, start=None, end=None, order=None, world=False, bvh = False,
        with_org_rotations=False):
    """
    ...
    if bvh:
        ...
        org_rotations = rotations
        rotations = Quaternions.from_euler(np.radians(rotations), order=order, world=world)
        #print np.linalg.norm(np.reshape(Quaternions.euler(rotations) -
        #    org_rotations, [-1]), 2)
        anim = (Animation(rotations, positions, orients, offsets, parents))
        
        if with_org_rotations:
            return anim, org_rotations
        else:
            return anim
    #return rotations
    
def joint_load(filename, src, org_rotations, aligned_ids, start=None, end=None, order=None, world=False, bvh = False):
    """
    Reads a BVH file and constructs an animation
    ...
    for line in f:
        ...
        if fmatch:
            if start and end:
                ...
                fnum = int(fmatch.group(1))
            fnum = org_rotations.shape[0]
            print 'get #frames=%d' % fnum
            jnum = len(parents)
            #positions = offsets[np.newaxis].repeat(fnum, axis=0)
            #rotations = np.zeros((fnum, len(orients), 3))
            positions = src.positions
            rotations = org_rotations
            continue
